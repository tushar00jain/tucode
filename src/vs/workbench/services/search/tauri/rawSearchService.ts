...
import { createChannelFileSearchEngine } from './channelFileSearchEngine.js';
...
export class SearchService implements IRawSearchService {
	...
	private readonly engineClass: { new(config: IFileQuery): ISearchEngine<IRawFileMatch> };

	constructor(
		raw: IFileSearchChannel,
		private readonly textSearchProvider: TextSearchProvider2,
		...
		this.engineClass = createChannelFileSearchEngine(raw);
	}
		...
		const emitter = new Emitter<ISerializedSearchProgressItem | ISerializedSearchComplete>({
			onDidAddFirstListener: () => {
				promise = createCancelablePromise(async token => {
					...
					return this.doFileSearchWithEngine(this.engineClass, query, p => emitter.fire(p), token, SearchService.BATCH_SIZE, numThreads);
				});
	...
	private ripgrepTextSearch(config: ITextQuery, progressCallback: IProgressCallback, token: CancellationToken): Promise<ISerializedSearchSuccess> {
		...
		const engine = new TextSearchEngineAdapter(config, this.textSearchProvider, this.fileService);

		return engine.search(token, progressCallback);
	}
	...
	doFileSearch(config: IFileQuery, numThreads: number | undefined, progressCallback: IProgressCallback, token?: CancellationToken): Promise<ISerializedSearchSuccess> {
		return this.doFileSearchWithEngine(this.engineClass, config, progressCallback, token, SearchService.BATCH_SIZE, numThreads);
	}
