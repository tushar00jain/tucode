...
// Copied from `addons/isl/src/relativeDate.tsx` by `scripts/copy-from-sapling.ps1` - the formatter, without the React component that wraps it. Do
// not edit: a run rewrites this file. Everything below the preamble is upstream's text,
// unchanged; a row's ``.commit-date`` renders ``relativeDate(date, { useShortVariant: true })``, which is what ``CommitDate`` in ``Commit.tsx`` passes.
