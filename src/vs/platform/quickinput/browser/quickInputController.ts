...
import { QuickInputControllerBase } from '../common/quickInputController.js';
...
import { IInputBox, IKeyMods, IQuickInput, IQuickInputButton, IQuickNavigateConfiguration, IQuickPick, IQuickPickItem, IQuickWidget, QuickInputHideReason, QuickPickFocus, QuickInputType, IQuickTree, IQuickTreeItem, QuickInputAlignment } from '../common/quickInput.js';
...
export class QuickInputController extends QuickInputControllerBase {
	private static readonly MAX_WIDTH = 600; // Max total width of quick input widget

	private idPrefix: string;
	private readonly onDidChangeEmitter = this._register(new Emitter<void>());
	/** Paint invalidation for hosts rendering the widget's DOM outside a browser. */
	readonly onDidChange = this.onDidChangeEmitter.event;

	/** Explicit viewport for hosts without DOM measurement. Browser layout is unchanged. */
	layoutList(height: number, width: number): void {
		this.ui?.list.layout(undefined, { height, width });
	}
	...
	private getUI(showInActiveContainer?: boolean): QuickInputUI {
		...
		this._register(list.onDidChangeSelection(() => this.onDidChangeEmitter.fire()));
		this._register(list.onDidChangeFocus(() => this.onDidChangeEmitter.fire()));
		this._register(inputBox.onDidChange(() => this.onDidChangeEmitter.fire()));
		this.ui = {
			didChange: () => this.onDidChangeEmitter.fire(),
			container,
			styleSheet,
			leftActionBar,
			titleBar,
