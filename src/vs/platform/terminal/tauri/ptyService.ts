...
import { TauriTerminalProcess } from './tauriTerminalProcess.js';
...
export class PtyService extends Disposable implements IPtyService {
	...
	// `refreshIgnoreProcessNames` is optional on `IPtyService` and nothing in this port calls
	// it: the child process monitor is the backend's, so the name list is `tscode_pty`'s.
	...
	/**
	 * Required by `IPtyService`, but the `pty` channel has no arm behind it — listing the
	 * processes holding a port is `netstat`/`lsof`, which is the backend's side of the split.
	 * Its only caller is the port-detection quick fix, which this port does not surface.
	 */
	@traceRpc
	async freePortKillProcess(port: string): Promise<{ port: string; processId: string }> {
		throw new ErrorNoTelemetry('freePortKillProcess is not implemented by the pty channel');
	}
	...
	async getDefaultSystemShell(osOverride: OperatingSystem = OS): Promise<string> {
		return this._channel.call<string>('getDefaultSystemShell', { osOverride });
	}

	@traceRpc
	async getEnvironment(): Promise<IProcessEnvironment> {
		return this._channel.call<IProcessEnvironment>('getEnvironment');
	}

	/**
	 * Stock spells this out over `wsl.exe` and the Windows build number; both are the
	 * backend's, so the whole method is one channel call and `_getWSLExecutablePath` goes with
	 * it.
	 */
	@traceRpc
	async getWslPath(original: string, direction: 'unix-to-win' | 'win-to-unix' | unknown): Promise<string> {
		if (direction !== 'win-to-unix' && direction !== 'unix-to-win') {
			// Fallback just in case
			return original;
		}
		return this._channel.call<string>('getWslPath', { original, direction });
	}
...
class PersistentTerminalProcess extends Disposable {
	...
	constructor(
		...
		private readonly _terminalProcess: TauriTerminalProcess,
		...
		this._serializer = new TerminalSerializer(cols, rows);
		if (name) {
