...
/**
 * Where the bundle keeps the shell integration scripts.
 *
 * Stock reads it from `FileAccess.asFileUri('')`, which needs the `_VSCODE_FILE_ROOT` global
 * this port deliberately leaves unset — see `docs/ARCHITECTURE.md`. Boot supplies the app
...
export async function getShellIntegrationInjection(
	...
	const windowsBuildNumber = isWindows ? await channel.call<number>('getWindowsBuildNumber') : 0;
	...
	switch (shell) {
		...
		case 'zsh': {
			...
			// Stock reads the user, the tmp directory and the home directory from `os`; the
			// renderer has none, so they come from the backend process environment.
			...
			// `chmod`, so the mode goes through the `file` channel's own arm; a no-op on Windows.
			if (!skipStickyBit) {
				try {
			...
			const userZdotdir = env?.ZDOTDIR ?? backendEnv['HOME'] ?? `~`;
